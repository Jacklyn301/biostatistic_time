...
```
* 在此處，`lower.tail=FALSE`代表要算右尾，因為R一般來說都默認左尾(畢竟都是從左尾累積...) 🙃
---
## CH3
...
![image alt](https://static.wingify.com/gcp/uploads/sites/3/2020/12/graphical-representation-of-type-1-and-type-2-errors.png)
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/type_1%262_error_graph_0314.png)
...
![image alt](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/QQplot_0314.png)
...
```
* 假說如下，要記得我們現在在**假設此為真的情況，所以兩樣本平均差為0**:
...
=\frac{(618-551)-(0)}{\sqrt[]{\frac{180^2}{13}+\frac{120^2}{7}}}=0.993
\end{align}
$$
...
##### 確認變異數的栗子時間 🌰
* 同樣回到綠鬣蜥那一題，我們不知道 $\sigma$ ，但是我告訴你: $s^2_A=37853.2,\,s^2_B=15037.0$ ，要先做f檢定。我們先設定好假設:
...
* 我們把服用前體重-服用後體重，然後將其體重差異標為d，這樣，假說可以被設置如下

$$\begin{align}
& H_0\rightarrow \mu_d\ge 5\\
& H_a\rightarrow \mu_d < 5
\end{align}$$
...
![Screenshot 2025-11-18 111314](https://raw.githubusercontent.com/Jacklyn301/image_bank/main/Sb%26F_0314.png)
...
\begin{align}
&SS_w\\
&=(2.5-2.44)^2+(3.1-2.44)^2+(2.3-2.44)^2+\\ & \quad (1.9-2.44)^2+(2.4-2.44)^2 +\ldots \text{(and so on)}\\
&=2.44
\end{align}$$
...
$$
\begin{array}{l c r}
&t_{u,90\%}=\frac{3.18-2.44}{\sqrt[]{0.203(\frac{1}{5}+\frac{1}{5})}}=2.5884\\
&&t_{90\%, 80\%}=\frac{3.54-3.18}{\sqrt[]{0.203(\frac{1}{5}+\frac{1}{5})}}=1.2592\\
...
&HSD=3.77\times\sqrt[]{\frac{0.203}{5}}=0.76\\
...
&\text{critical}_{(k=3, df=12)}= 3.77\times\sqrt[]{\frac{0.203}{5}}=0.76\\
...
&=\frac{12}{25\times 26}(\frac{162^2}{9}+\frac{84^2}{7}+\frac{79^2}{9})-3\times 26 =7.25
\end{align}
$$
...
* 所以一共需要檢差三個p-value
* 共有四個SS，包含 $SS_A, SS_B, SS_{AB}, SS_w$
...
SS_A=\sum_in_{i\cdot}(\bar{x}_{i\cdot}-\bar{x})^2
\end{array}
$$
...
SS_B=\sum_jn_{\cdot j}(\bar{x}_{\cdot j}-\bar{x})^2
\end{array}
$$
...
SS_{AB}=\sum_i \sum_j n_{ij}(\bar{x}_{ij}-\bar{x}_{i\cdot}-\bar{x}_{\cdot j}+\bar{x})^2
\end{array}
$$
...
$$
\begin{array}{l}
Y_{ijk}=\mu+\alpha_i+\beta_i+(\alpha\beta)_{ij}+\varepsilon_{ijk}\\
\end{array}$$
...
H_a\rightarrow\alpha_i\text{不全為}0
\end{array}
$$
...
H_a\rightarrow\beta_j\text{不全為}0
\end{array}
$$
...
* 咱們接下來來一個一個算... 🐱
* 先算SST (看完你就會知道為何C很重要了):
$$
SS_T = \sum x^2 - C = 1978 - 1875 = \boxed{103}
$$
...
&SS_A = \frac{73^2}{6} + \frac{77^2}{6} - 1875 ≈ 1876.3333 - 1875 = \boxed{1.33}
\end{align}
$$

- 接下來是SSB，每個B水準，有2個A × 2重複，所以一共 4 個觀測值，分母用4:

$$
\begin{align}
&SS_B = \sum \frac{T_{\cdot j}^2}{n_{\cdot j}} - C\rightarrow\\
&SS_B = \frac{38^2}{4} + \frac{48^2}{4} + \frac{64^2}{4} - 1875 =361 + 576 + 1024 - 1875 = \boxed{86}
\end{align}
$$

- 接下來要算SSAB，接下來的作法有點複雜，先看我做一次:
> 簡單一句話: 先做出SS_cells，再來做SSAB !!

$$
\begin{align}
&SS_{cells} = \sum \frac{T_{ij}^2}{n_{ij}} - C
\end{align}
$$
...
&df_W = ab(n-1) = 2 \times 3 \times (2-1) = 6 \\
&df_T = N - 1 = 12 - 1 = 11 \\
&\text{全部加起來： } 1 + 2 + 2 + 6 = 11\text{，計算正確}
\end{align}
$$
...
$$
\begin{align}
&F_A = \frac{MS_A}{MS_W} = \frac{1.33}{0.5} = 2.66 \\
&F_B = \frac{MS_B}{MS_W} = \frac{43}{0.5} = 86 \\
...
$$
\begin{align}
& SS_R=\beta_1^2(\sum X^2-n\bar{X}^2)=1.78^2\times 240=760.42\\
& SS_T=\sum Y^2-\frac{(\sum Y)^2}{n}=4365-\frac{179^2}{9}=804.89\\
...
$$
\begin{align}
& df_R=1 && MS_R=\frac{SS_R}{df_R}=760.42\\
& df_T=9-1=8 &&  MS_E=\frac{SS_E}{df_E}=6.35\\
& df_E=8-1=7 && F=\frac{MS_R}{MS_E}=119.75
\end{align}
$$
...
$$\chi^2_{df}=\frac{\sum(x_i-\bar{X})^2}{\sigma^2}=\frac{SS}{\sigma^2}$$
...
$$\chi^2=\sum^k_{i=1}\frac{(O_i-Ei)^2}{Ei}$$
...
H_a\rightarrow \text{數據跟孟德爾模型不一致}\\
\text{此時自由度為:}\, 3-1=2
\end{array}
$$
...
$$
\begin{align}
& P (X=k)=\frac{e^{-\lambda}\lambda^k}{k!}, \quad\text{其中:}\quad \lambda=\mu\\
...
& H_a\rightarrow \text{外殼種類與是否被占用兩者有關聯}\\
\end{align}
$$
...
$$
\begin{align}
\chi^2 &=\sum^{ij}\frac{(O_{ij}-E_{ij})}{E_{ij}}\\
& =\frac{(47-51.59)^2}{51.59}+\frac{(10-29.56)^2}{29.56}+\frac{(152-100.85)^2}{100.85}\\
& \quad +\frac{(42-37.41)^2}{37.41}+\frac{(41-21.44)^2}{21.44}+\frac{(49-73.15)^2}{73.15}\\
& =45.52
\end{align}
$$
...
& H_a\rightarrow 腎臟是否損傷跟團隊種類互相影響
\end{align}
$$
- 然後進行計算，自由度為:
...
$$
\begin{align}
\chi^2 & =\frac{4615\left | 454\times 2054-1692\times 415 \right |- 0.5\times 4615}{2146\times 2469\times 869 \times 3746}\\ &=13.91\\
